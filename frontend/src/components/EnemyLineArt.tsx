...
export function EnemyLineArt({
  ...
  // 2. Loading Box ONLY for the active latest turn while waiting for AI image to arrive
  if (isLatest) {
    return (
      <div className={`flex justify-center py-3 ${className}`} aria-hidden>
  ...
  // 3. For past turns without image, return null (never show loading box for old turns)
  return null;
}

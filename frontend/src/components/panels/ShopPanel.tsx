...
// Fallback SKUs from pool.json in case server returned empty list
const FALLBACK_ITEMS: ShopSku[] = pool.skus.map((s, idx) => ({
  sku: s.sku,
  title: s.title,
  description: s.description,
  priceTomans: Number(s.price) || 1000,
  type: s.sku === 'scenario_kavir' || s.sku === 'unlock_full_ui' ? 'non_consumable' : 'consumable',
  sortOrder: idx + 1,
  isActive: true,
}));
